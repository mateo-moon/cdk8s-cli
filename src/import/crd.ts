 ...
 * utility type. This function adds a type alias and replaces usages to fix the conflict.
 *
...
function fixBuiltinTypeConflicts(renderedCode: string, kind: string): string {
  ...
  // Add a type alias at the beginning of the code to preserve the built-in type
  const typeAlias = `// Type alias to avoid collision with the ${kind} class defined below\ntype Json${kind}<K extends keyof any, T> = { [P in K]: T };\n\n`;
  ...
  const fixedCode = renderedCode.replace(pattern, `Json${kind}<$1>`);

  return typeAlias + fixedCode;
}
